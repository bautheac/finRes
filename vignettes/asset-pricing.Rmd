...
```{r `pullit equity storethat`, message = F, warning = F, echo = F, eval = F}
library(pullit); library(lubridate)
...
```{r `factorem factor`, message = F, warning = F, eval = F}
library(factorem)
...
```{r `plot performance`, fig.width = 7.5, fig.height = 5.5, fig.fullwidth = T, eval = F}
library(plotit)
...
#### positions overview
```{r `plot positions`, fig.width = 7.5, fig.height = 6.5, fig.fullwidth = T, eval = F}
