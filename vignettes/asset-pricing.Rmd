...
```{r `pullit equity BBG`, message = F, warning = F}
...
```{r `factorem factor`, message = F, warning = F, eval = T}
library(factorem)
...
```{r `pullit futures`, message = F, warning = F}
...
```{r `futures CHP`, message = F, warning = F}
...
```{r `futures OI aggregate`, message = F, warning = F}
...
#### name
```{r `factor name`, eval = T}
get_name(factor)
```
#### positions
```{r `factor positions`, eval = T}
get_positions(factor)
```
#### returns
```{r `factor returns`, eval = T}
get_returns(factor)
```
#### data initially supplied
```{r `factor data`, eval = T}
get_data(factor)
```
#### parameters
```{r `factor params`, eval = T}
get_parameters(factor)
```
#### function call
```{r `factor call`, eval = T}
get_call(factor)
```
...
```{r `plot performance`, fig.width = 7.5, fig.height = 5.5, fig.fullwidth = T, eval = T}
library(plotit)
...
#### positions overview
```{r `plot positions`, fig.width = 7.5, fig.height = 6.5, fig.fullwidth = T, eval = T}
