...
All the functions described above can be used to query data from a [storethat](https://bautheac.github.io/storethat/) database by setting the `source` parameter to "storethat" and providing the path to the database as a string in the `file` parameter. If omitted a pop-up window allows to browse the file system and select the database file. The other function parameters are identical for both data sources and the returned objects are identical in all aspects other than the source of the data they contain:  
