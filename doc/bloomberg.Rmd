...
```{r `globals`, warnings = FALSE, message = FALSE, eval = TRUE}
...
```{r `fund market storethat`, echo = FALSE, eval = TRUE}
fund_tickers <- c("SPY US Equity", "GLD US Equity", "EEM US Equity")
...
```{r `futures term structure storethat`, echo = FALSE, eval = TRUE}
futures_tickers <- c("C A Comdty", "EDA Comdty", "ESA Index")
...
```{r `futures show`, eval = TRUE}
futures_TS
```

Access each slot using the appropriate accessor:
```{r `futures get_tickers`, eval = TRUE}
get_active_contract_tickers(futures_TS)
```

```{r `futures get_fields`, eval = TRUE}
get_fields(futures_TS)
```

```{r `futures get_data`, eval = TRUE}
pullit::get_data(futures_TS)
```

```{r `futures get_call`, eval = TRUE}
pullit::get_call(futures_TS)
```

```{r `futures get_periods`, eval = TRUE}
...
```{r `plot term structure`, fig.width = 7.5, fig.height = 5.5, fig.fullwidth = TRUE, eval = TRUE}
library(plotit)
...
```{r `plot performance`, fig.width = 7.5, fig.height = 6.5, fig.fullwidth = TRUE, eval = TRUE}
